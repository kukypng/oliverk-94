...
export const AdminTestPanel = () => {
  ...
  // Test users query
  const { data: usersTest, isLoading: usersLoading, refetch: refetchUsers } = useQuery({
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Button 
              onClick={refreshAllTests} 
              variant="outline" 
              size="sm"
              disabled={debugLoading || testLoading || usersLoading}
            >
        ...
        <CardContent className="space-y-6">
          {/* Debug Info Section */}
          <div className={`p-4 rounded-lg border ${getStatusColor(debugInfo?.is_admin)}`}>
            ...
            {debugLoading ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><strong>Status:</strong> {
                  debugInfo.is_admin ? 'Administrador Ativo' : 
                  debugInfo.user_role === 'admin' ? 'Admin Inativo' : 
                  'Usuário Regular'
                }</div>
              </div>
            ) : (
              <p className="text-sm text-red-600">Erro ao carregar informações do usuário</p>
            )}
          </div>
            ...
            {testLoading ? (
              ...
              <div className="space-y-2">
                {testResults.map((test, index) => (
                  <div key={index} className={`p-3 rounded-lg border ${getStatusColor(test.result)}`}>
              ...
              <p className="text-sm text-yellow-600">Nenhum teste disponível</p>
            )}
          </div>

          {/* Users Access Test Section */}
          <div className={`p-4 rounded-lg border ${getStatusColor(usersTest?.success)}`}>
            <h3 className="font-semibold mb-3 flex items-center">
              {getStatusIcon(usersTest?.success)}
              <span className="ml-2">Teste de Acesso aos Usuários</span>
            </h3>
            {usersLoading ? (
              <p className="text-sm text-muted-foreground">Testando acesso...</p>
            ) : usersTest ? (
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm space-y-1">
              <p><strong>Status do Admin:</strong> {
                debugInfo?.is_admin ? (
                  <span className="text-green-600">Funcionando corretamente</span>
                ) : debugInfo?.user_role === 'admin' ? (
                  <span className="text-yellow-600">Usuário admin existe mas está inativo</span>
                ) : (
                  <span className="text-red-600">Usuário não é administrador</span>
                )
              }</p>
              <p><strong>Acesso aos Usuários:</strong> {
                usersTest?.success ? (
                  <span className="text-green-600">Funcionando ({usersTest.count} usuários)</span>
                ) : (
                  <span className="text-red-600">Bloqueado</span>
                )
              }</p>
              <p><strong>Testes de Permissão:</strong> {
                testResults ? (
                  <span className="text-green-600">{testResults.filter(t => t.result).length}/{testResults.length} passaram</span>
                ) : (
                  <span className="text-yellow-600">Não executados</span>
                )
              }</p>
            </div>
          </div>
