...
export const MobileQuickAccess = ({ onTabChange, stats }: MobileQuickAccessProps) => {
  const quickActions = [
    {
      id: 'new-budget',
      icon: PlusCircle,
          ...
          {quickActions.map((action) => {
            ...
            return (
              <Button
                key={action.id}
                variant="outline"
                onClick={() => action.action ? action.action() : onTabChange(action.id)}
        ...
        {stats && (
          <div className="mt-4 pt-4 border-t border-border/50">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">{stats.totalBudgets}</span>
            </div>
